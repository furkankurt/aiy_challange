...
def joy_detector(num_frames, preview_alpha, image_format, image_folder,
                 enable_streaming, streaming_bitrate, mdns_name, client):
    ...
    with contextlib.ExitStack() as stack:
        ...
        for faces, frame_size in run_inference(num_frames, model_loaded):
            ...
            if event == 'high':
                ...
                client.publish("fkurt97/feeds/AIY/", "speak Happy.")
                time.sleep(1) # wait
            elif event == 'low':
                logger.info('Low joy detected.')
                player.play(SAD_SOUND)
                client.publish("fkurt97/feeds/AIY", "speak Sad.")
                time.sleep(1) # wait
...
def main():
    ...
    user = "fkurt97"
    password = "aio_TnJU484v2O6PhKxgz46yvUBEagf5"
    ...
    try:
        joy_detector(args.num_frames, args.preview_alpha, args.image_format, args.image_folder,
                     args.enable_streaming, args.streaming_bitrate, args.mdns_name, client)
